...
| Severity | Backend | Anki Add-on | Cross-cutting | Total |
|----------|---------|-------------|---------------|-------|
| Critical | 2 | 1 | — | 3 |
| Important | 8 | 8 | 1 | 17 |
| Consider | 8 | 5 | — | 13 |
...
**Critical** findings are security vulnerabilities or data-loss risks that should be addressed before the web app ships. **Important** findings are contract mismatches, missing PRD features, or reliability gaps that will cause confusion or bugs with two clients. **Consider** findings are code quality issues that won't block launch but increase maintenance cost.
...
#### X-1: Tier naming mismatch between PRD and code
...
#### B-C1: CORS allows all origins on authenticated endpoints
...
#### B-C2: Rate limiting is in-memory only — resets on Worker restart
...
#### B-I1: Claude timeout documented as 30s but implemented as 60s
...
#### B-I2: PRD rate limits not implemented — only simple per-minute cap exists
...
#### B-I3: Missing `CONFLICT` error code in ErrorCode type
...
#### B-I4: Client-supplied `X-Request-ID` is accepted without validation
...
#### B-I5: TOCTOU race condition in usage checking
...
#### B-I6: Webhook handlers leak internal error details
...
#### B-I7: Content-Length bypass — middleware doesn't verify actual body size
...
#### B-I8: `multipart/form-data` always gets 10MB limit regardless of route
...
#### B-M1: Claude 529 (Overloaded) not distinguished from generic 5xx
...
#### B-M2: Stripe SDK has no explicit timeout — defaults could exceed Worker CPU limit
...
#### B-M3: `recordUsage()` is fire-and-forget — silent data loss under Supabase degradation
...
#### B-M4: Portal `return_url` from query param with no origin validation — open redirect risk
...
#### B-M5: Checkout `success_url` and `cancel_url` not origin-validated
...
#### B-M6: Usage route returns 503 instead of 500 for internal errors
...
#### B-M7: `/auth/me` returns minimal data — web app will need subscription tier
...
#### B-M8: `parseSubscriptionTier()` silently downgrades unknown tiers to `free`
...
#### A-C1: JWT tokens stored as plaintext in Anki config (SQLite)
...
#### A-I1: `tts_back_url` dead code path in generate_with_assets
...
#### A-I2: TTS download bypasses `client.request()` — missing 401-refresh and 429-retry
...
#### A-I3: Login call suppresses `product_source` — violates API contract
...
#### A-I4: Enhancement processor downloads TTS/images on main thread
...
#### A-I5: No `retry_after` cap on 429 retry — server could force arbitrary sleep
...
#### A-I6: TTS preview temp files never deleted — resource leak
...
#### A-I7: `image_attribution` rendered unescaped in QLabel — Qt rich text injection
...
#### A-I8: No guard against Anki sync interleaving with active API calls
...
#### A-M1: ~33 `Any` type annotations violate "no Any" rule
...
#### A-M2: `token_expires_at` stored but never proactively checked
...
#### A-M3: `has_stored_auth()` only checks token existence, not validity
...
#### A-M4: Temporary TTS/image bytes held in memory, never explicitly freed
...
#### A-M5: Client timeout documentation mismatch
...
## Appendix: Code Simplification Suggestions

Simplification suggestions for files with only Consider-level findings. No source files were modified.

### `flashcard-backend/src/services/stripe.ts`

**S1. Placeholder comment on `METER_EVENT_NAME` (line 31)**
...
**S2. Missing Stripe SDK `timeout` configuration (lines 15-18)**
...
**S3. `getPriceIdForTier` is a trivial single-line wrapper (lines 42-44)**
...
**G1. Duplicate `DomainConfig` interface (lines 30-37)**
...
**G2. No-op `as unknown[]` casts (lines 295, 319)**
...
**G3. Spread-conditional pattern for optional response fields (lines 316-317)**
...
**A1. Broad `except Exception` on `enhance_cards` (line 137) — intentionally correct**
...
**A2. Four unreachable `except Exception` blocks on download calls (lines 170, 191, 204, 214)**
...
**A3. Dead `tts_back_url` code path (lines 175-184)**
...
**A4. Redundant `or ""` fallback (line 210)**
...
**A5. TTS direction filter placed after list assignment (lines 219-226)**
